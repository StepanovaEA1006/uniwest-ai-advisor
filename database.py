...
# ДАННЫЕ О ПОДПИСКАХ КЛИЕНТОВ - ОБНОВЛЕННЫЕ ДАННЫЕ!
CLIENT_SUBSCRIPTIONS = {
    'Иван Петров': {'level': 'premium', 'price': 800, 'expires': '2024-12-31'},
    'Мария Сидорова': {'level': 'advanced', 'price': 500, 'expires': '2024-11-30'},
    'Алексей Козлов': {'level': 'basic', 'price': 300, 'expires': '2024-10-15'},
    'Елена Волкова': {'level': 'trial', 'price': 0, 'expires': '2024-09-30'},
    'Дмитрий Смирнов': {'level': 'basic', 'price': 300, 'expires': '2024-09-30'}
}

# ОПИСАНИЯ ТАРИФОВ И ФУНКЦИЙ
SUBSCRIPTION_FEATURES = {
    'trial': {
        ...
        'features': [
            ...
            '💡 Общие рекомендации',
            '📈 Простая аналитика доходности',
            '📱 Доступ с мобильных устройств'
        ],
        'limitations': [
            '❌ Без глубокого анализа рисков',
            '❌ Без оптимизации портфеля',
    ...
    'advanced': {
        ...
        'features': [
            ...
            '📰 Новостной анализ активов',
            '⚡ Тактические рекомендации',
            '📊 Сравнительная аналитика',
            '📈 Расширенные метрики (Сортино, Калмар)'
        ],
        'upgrade_reason': 'Для максимальной доходности при контроле рисков'
    },
    'premium': {
        'name': 'Премиум',
        'price': 800,
        'features': [
            ...
            '🏆 Сравнение с эталонными портфелями',
            '🚀 Приоритетная поддержка 24/7',
            '📈 Расширенная аналитика',
            '💎 Персональный финансовый советник',
            '🎯 Эксклюзивные инвестиционные идеи'
        ],
        'upgrade_reason': 'Для профессиональных инвесторов и максимальных результатов'
    }
}
...
def get_subscription_level(client_name: str) -> str:
    ...
    return subscription.get('level', 'trial')
...
PREMIUM_ANALYTICS_DATA = {
    'Иван Петров': {
        ...
        'benchmark_comparison': {
            'sp500': {'return': 0.12, 'volatility': 0.15},
            'nasdaq': {'return': 0.18, 'volatility': 0.22},
            'russian_index': {'return': 0.08, 'volatility': 0.18},
            'your_portfolio': {'return': 0.15, 'volatility': 0.20}
        },
        'ml_insights': [
            '📈 Высокая корреляция с технологическим сектором',
            '⚡ Портфель перевешен в акции роста',
            '🛡️ Рекомендуется добавить защитные активы',
            '🎯 Оптимальный момент для ребалансировки'
        ]
    },
    'Мария Сидорова': {
        'ai_predictions': {
        ...
        'benchmark_comparison': {
            ...
            'nasdaq': {'return': 0.18, 'volatility': 0.22},
            'russian_index': {'return': 0.08, 'volatility': 0.18},
            'your_portfolio': {'return': 0.10, 'volatility': 0.16}
        },
        'ml_insights': [
    ...
    'Алексей Козлов': {
        'ai_predictions': {
            'next_month_return': 0.032,
            'confidence': 0.75,
            'trend': 'bullish',
            'key_drivers': ['диверсификация портфеля', 'стабильность дивидендов'],
            'risk_warnings': ['изменение процентных ставок', 'валютные риски']
        },
        'benchmark_comparison': {
            'sp500': {'return': 0.12, 'volatility': 0.15},
            'nasdaq': {'return': 0.18, 'volatility': 0.22},
            'russian_index': {'return': 0.08, 'volatility': 0.18},
            'your_portfolio': {'return': 0.095, 'volatility': 0.14}
        },
        'ml_insights': [
            '⚖️ Идеальный баланс роста и стабильности',
            '🌍 Отличная географическая диверсификация',
            '💵 Стабильный дивидендный поток',
            '📈 Потенциал роста 8-12% годовых'
        ]
    }
}
...
# ФУНКЦИЯ ДЛЯ РЕКОМЕНДАЦИЙ С УЧЕТОМ ПОДПИСКИ - ИСПРАВЛЕННАЯ ВЕРСИЯ
def generate_subscription_based_recommendations(client_name: str) -> List[str]:
    ...
    subscription_messages = {
        'trial': [
            "🔒 **Ограничения пробного периода**: Доступны только базовые рекомендации",
            "🚀 **Перейдите на Базовый тариф**: Получите персонализированные рекомендации за 300 руб/мес"
        ],
        'basic': [
            "📊 **Базовый тариф**: Доступны основные функции анализа",
            "🎯 **Улучшите до Продвинутого**: Получите оптимизацию портфеля и анализ рисков за 500 руб/мес"
        ],
        'advanced': [
    ...
    if level == 'trial':
        return base_recommendations[:2] + subscription_messages['trial']
